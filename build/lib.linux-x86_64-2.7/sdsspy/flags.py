...
class Flags():
    ...
    def get_prefix(self, prefix):
        ...
        if wprefix.size == 0:
            raise  ValueError("No match for prefix: '%s'" % prefix)
        return flags[wprefix]
...
def printflags(flags, prefix):
    """
    Print the flags for a single object
    """
    f = Flags()
    fs = f.get_prefix(prefix)

    setones=[]
    maxlab=0
    for i in xrange(fs.size):
        val = 2**fs['bit'][i]

        if (flags & val) != 0:
            maxlab = max(maxlab, len(fs['label'][i])) 
            setones.append({'label':fs['label'][i], 'desc':fs['description'][i]})
    if len(setones) != 0:
        f='%-'+str(maxlab)+'s %s'
        for so in setones:
            print(f % (so['label'],so['desc']))
