...
static U16 *ai_pix_set(U16 *pix, const OBJMASK *om, const REGION *reg,
		       int drow, int dcol);
static void ai_reg_set(const OBJMASK *om, const U16 *pix, REGION *reg,
		       int drow, int dcol, int sky);
		 ...
		 int deep)		/* do a deep copy? */
{
   ...
   for(c = 0;c < ncolor;c++) {
      ...
      ai->pix[c] = shMalloc(old->npix*sizeof(U16));
      memcpy(ai->pix[c], old->pix[c], old->npix*sizeof(U16));
			  ...
			  float sky)	/* add this to all ai's pixels */
{
   ...
   ai_reg_set(ai->master_mask, ai->pix[c], reg,
	      ai->drow[c] - row0, ai->dcol[c] - col0, (int)(sky + 0.5));
}
 ...
 */
static U16 *
ai_pix_set(U16 *pix,			/* space to hold the pixels, or NULL */
 ...
 */
   if(pix == NULL) {
      pix = shMalloc(om->npix*sizeof(U16));
   }
/*
   ...
   for(i = 0;i < om->nspan;i++) {
	 ...
	 if(x2 >= ncol) {		/* span extends to right of region */
	    ...
	    if(ncopy > 0) {		/* x1 is < ncol */
	       memset(&ptr[ptr0 + ncopy],'\0',x2*sizeof(U16));
	    }
	 }
	 
	 if(ncopy > 0) {
	    rptr = &rows[y][col0];
	    memcpy(&ptr[ptr0],rptr,ncopy*sizeof(U16));
	 }
      } else {
	 memset(ptr,'\0',npix*sizeof(U16));
      }
      ptr += npix;
   }
...
ai_reg_set(const OBJMASK *om,		/* the OBJMASK specifying the pixels */
	   ...
	   int drow, int dcol,		/* how much to offset om's coords */
	   int sky)			/* add this value to all pixels */
{
    int i;
    int npix;				/* number of pixels in OBJMASK */
    U16 **rows;				/* == reg->rows */
    ...
    for(i = 0;i < om->nspan;i++) {
        ...
        if(y >= 0 && y < nrow) {
            ...
            if(ncopy > 0) {
                if(sky == 0) {
                    memcpy(&rows[y][col0],&ptr[ptr0],ncopy*sizeof(U16));
                } else {
                    int j;
                    for(j = 0; j < ncopy; j++) {
		...
		int nbad)		/* number of bad sections */
{
   ...
   U16 **pix = aimage->pix;		/* data arrays */
 ...
 */
   ...
   for(i = 0; i < ngood; i++) {
      ...
      if(isrc != idst) {
	 for(c = 0; c < ncolor; c++) {
	    memmove(&pix[c][idst], &pix[c][isrc], n*sizeof(U16));
	 }
      }
      idst += n;
   }
...
/*****************************************************************************/
/*
 * This routine isn't actually called in photo, but it's very handy when
 * called from gdb
 *
 * Return the index in the ->pix arrays of the pixel (row, col)
 */
int
p_phFindAtlasImagePixel(const ATLAS_IMAGE *aimage, /* the atlas image */
			int row, int col) /* the desired pixel */
{
   ...
   for(i = n = 0; i < mmask->npix; i++) {
      if(mmask->s[i].y < row || mmask->s[i].x1 > col) {
	 n += mmask->s[i].x2 - mmask->s[i].x1 + 1;
	 continue;
      } else if(mmask->s[i].y > row || mmask->s[i].x2 < col) {
