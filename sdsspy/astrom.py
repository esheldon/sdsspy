...
class AstromAstrans(object):
    ...
    camcol:
        The camera column id
    filter:
        The filter number or character
    rerun: optional
        This will be found from the run if the run is in
        the runList.par file
    ...
    * For the inverse transforms from ra,dec to pixels you need
    ...
    def pix2eq(self, field, row, col, color=0.3):
        """
        convert from pixel coordinates (row,col) to equatorial coordinates
        (ra,dec)
    ...
    def munu2pix(self, field, mu, nu, color=0.3):
        ...
        if color.size > 1 and color.size !=mu.size:
            raise ValueError("color should be scalar or same size as inputs")
    ...
    def pix2munu(self, field, row, col, color=0.3):
        """
        convert from row-column to SDSS great circle coordinates (mu,nu)
        ...
        rowm = 0.5 + row+drow0+drow1*col+drow2*(col**2)+drow3*(col**3)+csrow*color
        colm = 0.5 + col+dcol0+dcol1*col+dcol2*(col**2)+dcol3*(col**3)+cscol*color
    ...
    def load(self):
        ...
        fname=files.filename('asTrans', **self.keys)

        info=util.get_id_info(**self.keys)
        ...
        wcam,=where(camcols==info['camcol'])
        if wcam.size==0:
            raise  ValueError("bad camcol: %s" % info['camcol'])
